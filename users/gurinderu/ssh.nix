{ config, ... }:
{
  programs.ssh = {
    ...
    settings = {
      ...
      # sing-box fakeip (whatever pool users/gurinderu/fakeip-range.nix
      # currently holds) and dead-ends in the proxy, and MagicDNS is off on
      # this host, so only the fixed 100.x address is reliable. Tailscale
